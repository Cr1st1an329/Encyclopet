...
switch($accion){
    ...
    case"AgregarPerro":
        $sentenciaSQL= $conexion->prepare("INSERT INTO donacion (raza,informacion,imagen) VALUES (:raza,:informacion,:imagen);");
        ...
        if($tmpImagen!=""){
                ...
                move_uploaded_file($tmpImagen,"../../imgDo/".$nombreArchivo);
    ...
    case"Modificar":
        ...
        $sentenciaSQL= $conexion->prepare("UPDATE donacion SET Raza=:Raza WHERE id=:id");
        $sentenciaSQL->bindParam(':Raza',$txtRaza);
        $sentenciaSQL->bindParam(':id',$txtID);
        $sentenciaSQL->execute(); 
        $sentenciaSQL= $conexion->prepare("UPDATE donacion SET Informacion=:Informacion WHERE id=:id");
        $sentenciaSQL->bindParam(':Informacion',$txtRazaInfo);
        $sentenciaSQL->bindParam(':id',$txtID);
        $sentenciaSQL->execute();
        ...
        if($txtImagen !=""){
            ...
            $sentenciaSQL= $conexion->prepare("SELECT donacion FROM mascotas WHERE id=:id");
            $sentenciaSQL->bindParam(':id',$txtID);
            $sentenciaSQL->execute(); 
            $mascota=$sentenciaSQL->fetch(PDO::FETCH_LAZY);
            if(isset($mascota["imagen"]) && ($mascota["imagen"]!="imagen.jpg")){
                 if(file_exists("../../imgDo/".$mascota["imagen"])){
                     unlink("../../imgDo/".$mascota["imagen"]);
                   }
            }


             $sentenciaSQL= $conexion->prepare("UPDATE donacion SET imagen=:imagen WHERE id=:id");
             $sentenciaSQL->bindParam(':id',$txtID);
             $sentenciaSQL->bindParam(':imagen',$nombreArchivo);
             $sentenciaSQL->execute(); 
        }
    ...
    case"Cancelar":
        ...
        break;
    case"Seleccionar":
        $sentenciaSQL= $conexion->prepare("SELECT * FROM donacion WHERE id=:id");
        $sentenciaSQL->bindParam(':id',$txtID);
        $sentenciaSQL->execute(); 
        $mascota=$sentenciaSQL->fetch(PDO::FETCH_LAZY);
        ...
        // echo"Presionado boton Seleccionar ";
            break;
    case"Borrar":
        $sentenciaSQL= $conexion->prepare("SELECT imagen FROM donacion WHERE id=:id");
        $sentenciaSQL->bindParam(':id',$txtID);
        $sentenciaSQL->execute(); 
        $mascota=$sentenciaSQL->fetch(PDO::FETCH_LAZY);
        if(isset($mascota["imagen"]) && ($mascota["imagen"]!="imagen.jpg")){
        ...
        $sentenciaSQL= $conexion->prepare("DELETE FROM donacion where id=:id");
        $sentenciaSQL->bindParam(':id',$txtID); 
        $sentenciaSQL->execute(); 
        header("Location:Donaciones.php");
         break;
...
$sentenciaSQL= $conexion->prepare("SELECT * FROM donacion");
$sentenciaSQL->execute(); 
...
<div class="col-md-12">
    ...
    <input type="file"  class="form-control" name="txtImagen" id="txtImagen"  placeholder="Imagen de la raza">
        ...
        <div class="btn-group" role="group" aria-label="">
            <button type="submit" name="accion"<?php echo ($accion=="Seleccionar")?"disabled":""; ?> value="AgregarPerro" class="btn btn-success">Agregar lugar de donaciones</button>
   ...
   <table class="table table-bordered">
        ...
        <?php foreach($listamascotas as $listamascotas){?>
            <tr>
                ...
                <img src="../../imgDo/<?php echo $listamascotas['imagen']?>" width="150" alt="" srcset="">
